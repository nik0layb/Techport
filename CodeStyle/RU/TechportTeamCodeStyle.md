...
Не определено единого стандарта для файлов __не определяющих__ PHP классов. Так как мы юзаем фреймворки для разработки кода, в первую очередь, мы должны соответствовать базовым стандартам, определенным их разработчиками. Приоритет стандарта, диктуемого разработчиком фреймворка __всегда выше__ общих стандартов разработки, за исключением отдельных случаев, которые могут подвергаться обсуждению командой.

Для каждого файла __определяющего__ один PHP класс:

По дефолту, путь к файлу выглядит следующим образом: __\<Кто\_Разработал>\<Дополнительные>\<Пространства\_Имен>\<Нужный\_Ресурс>.php__
