...
Имена __с расширением__ ".php", не определяющие класс, __без побочного эффекта__ (например helpers):
...
* _следует_ давать имена простые для восприятия и удобные для быстрого определения цели/контекста применения файла.
* _запрещено_ давать бессмысленные имена.
* _запрещено_ использование символа ":" (ASCII: 0x3A), т.к. этот символ используется в OSX для разделения пути к файлу.
