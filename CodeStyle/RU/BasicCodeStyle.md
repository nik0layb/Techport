...
Имена __с расширением__ ".php", содержащие в себе определение PHP класса:
...
Имена __с расширением __ ".php", не определяющие класс, без побочного эффекта (например helpers):

* _должно_ соответствовать under_score нотации.
* _должно_ содержать область применения.

```php
    include 'config.php';   # Конфигурационный файл Phalcon
    include 'services.php'; # Сервисы для Phalcon-контейнера.
```
...
```php
    include 'sys_helpers.php';  # Хелперы для работы с операционной системой.
    include 'http_helpers.php'; # Хелперы для работы с HTTP протоколом.
```

Имена __с расширением __ ".php", содержащие в себе побочный эффект:
...
Имена __с расширением__ ".php", не вошедшие в группу выше, считаются не стандартными (Почему? Указано дальше). 
...
Имена __без расширения__ ".php":
Именуются в свободном стиле.
