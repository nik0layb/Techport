...
##[0.5] Между небом и землей
Файлы с кодом PHP PSR-1 делит на две части: имеющие побочный эффект и не имеющие побочный эффект.
Под побочным эффектом имеется код, интерпретация которого меняет предыдущее состояние, в котором находилось приложение.
...
```php
    ...
    # Вызов функций (здесь это __construct).
    $se = new SideEffect();
    ...
    # Многое другое, что производит какие-либо действия, способные и/ или изменяющие, состояние приложения.
```
...
С одной стороны это не относится к терминологии, с другой стороны, это выделено стандартом, как важное составляющее.

##[0] Файлы->Именование
Файлы нашего проекта легко делятся на два типа: с расширением `.php`, и без расширения `.php`.

__C расширением__ "php", содержащие в себе определение PHP класса _должны_:

* Быть написаны в StudlyCaps нотации.
* Соответствовать названию класса, описанного в файле.

__C расширением __ "php", содержащие в себе активные действия.
